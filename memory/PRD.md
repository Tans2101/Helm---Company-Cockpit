...
- Data: single seeded company doc `helm-demo` in Mongo (seed_data.py).
...
- Telemetry, Financials, Tasks (drag Kanban), Reports (+AI Weekly CEO Pack), Team Bandwidth, Calendar, People, Ask Helm (streaming Claude), Integrations, Billing (Stripe test-mode).
...
- Resend integration: owner invites now send a branded (dark/gold) HTML "you've been added to Helm" email via Resend. send_invite_email() runs non-blocking (asyncio.to_thread) and degrades gracefully to a no-op when RESEND_API_KEY is empty. invite response returns email_sent flag; Members UI toast reflects instant-join / email-sent / pending-invite.
...
- P2: Ask Helm multi-turn memory carry-over; email/Slack delivery of morning briefing.
