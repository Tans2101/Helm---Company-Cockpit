...
export const HEATMAP_DEFAULT_ENTER_EASE = [0.85, 0, 0.916, 0.282];

/** Default enter transition for {@link HeatmapChart}. */
export const HEATMAP_DEFAULT_ENTER_TRANSITION = {
  type: "tween",
...
export function heatmapCellSeed(column, row) {
  return column * 1009 + row * 9176;
}

/** Min/max contribution levels present in the dataset. */
export function computeHeatmapLevelRange(data) {
...
export function resolveHeatmapEnterFadeDurationSec(enterTransition, animationDurationMs) {
  if (
    enterTransition &&
...
export function heatmapLoadingCellParticipates(column, row, randomness) {
  if (randomness >= 1) {
    return true;
  }
  if (randomness <= 0) {
