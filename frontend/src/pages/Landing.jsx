...
function BriefingPreview() {
  return (
    <div className="relative rounded-2xl border border-white/[0.08] bg-[#121214] p-5 md:p-7 border-l-4 border-l-gold">
      <div className="flex items-center justify-between">
        <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-gold">Monday · Morning Briefing</p>
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white overflow-x-hidden relative">
      ...
      {/* Hero — no radial glow; typography + product mockup carry the section */}
        ...
        <div className="relative mx-auto max-w-6xl grid lg:grid-cols-[1.04fr_0.96fr] gap-14 lg:gap-14 items-center">
          <div>
            ...
            <motion.h1 variants={fade} initial="hidden" animate="show" custom={1}
              className="font-serif-display mt-6 text-4xl sm:text-5xl lg:text-6xl font-medium tracking-[-0.02em] leading-[1.08]">
            ...
            <motion.div variants={fade} initial="hidden" animate="show" custom={3} className="mt-9 flex flex-wrap items-center gap-3 relative z-10">
              <button data-testid="hero-cta-btn" onClick={enter} type="button"
                className="group inline-flex items-center gap-2 rounded-lg bg-gold text-black font-medium px-6 py-3 transition-colors hover:bg-gold-hover">
                {authed ? "Open your cockpit" : "Start free"}
                <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
              </button>
              <a href="#how" className="inline-flex items-center gap-2 rounded-lg border border-white/10 px-6 py-3 text-sm text-zinc-300 transition-colors hover:bg-white/5">
      ...
      <section className="px-6 py-24 border-t border-white/[0.05]">
        <div className="mx-auto max-w-6xl">
          <motion.div variants={fade} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }} className="max-w-2xl">
            ...
            <h2 className="font-serif-display mt-4 text-3xl md:text-4xl font-medium tracking-tight leading-snug">From morning briefing to weekly update.</h2>
          </motion.div>
          <div className="mt-14 space-y-0 border-t border-white/[0.06]">
            {CEO_DAY.map((step, i) => (
              <motion.div key={step.title} variants={fade} custom={i} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-60px" }}
                className="grid sm:grid-cols-[7rem_1fr] gap-3 sm:gap-8 py-6 border-b border-white/[0.06]">
                <p className="font-mono text-[10px] uppercase tracking-wider text-gold pt-1">{step.time}</p>
                <div>
                  <h3 className="text-white font-medium">{step.title}</h3>
                  <p className="mt-1.5 text-sm text-zinc-400 leading-relaxed max-w-xl">{step.body}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section id="how" className="px-6 py-24 border-t border-white/[0.05]">
        <div className="mx-auto max-w-6xl">
          <motion.div variants={fade} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }} className="max-w-2xl">
            ...
            <h2 className="font-serif-display mt-4 text-3xl md:text-4xl font-medium tracking-tight">One short operating rhythm.</h2>
          </motion.div>
          <div className="mt-16 grid md:grid-cols-3 gap-10 md:gap-8">
            {HOW_IT_WORKS.map((s, i) => (
              <motion.div key={s.n} variants={fade} custom={i} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }}>
                ...
                <h3 className="mt-4 text-xl text-white tracking-tight">{s.title}</h3>
      ...
      <section className="px-6 py-24 border-t border-white/[0.05]">
        <div className="mx-auto max-w-6xl">
          <motion.div variants={fade} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }}>
            ...
            <h2 className="font-serif-display mt-4 text-3xl md:text-4xl font-medium tracking-tight max-w-2xl leading-tight">Everything a CEO needs — nothing they don&apos;t.</h2>
          </motion.div>
            ...
            {FEATURE_HIGHLIGHTS.map((f, i) => {
              ...
              return (
                <motion.div key={f.title} variants={fade} custom={i} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-60px" }}
                  className={lead ? "lg:col-span-2 border-b border-white/[0.06] pb-8" : "pt-1"}>
                  <div className={lead ? "max-w-2xl" : ""}>
                    ...
                    <p className={`text-sm text-zinc-400 leading-relaxed ${lead ? "text-base max-w-xl" : ""}`}>{f.body}</p>
                  </div>
                </motion.div>
              );
            })}
      ...
      <section id="pricing" className="px-6 py-24 border-t border-white/[0.05]">
        <div className="mx-auto max-w-6xl">
          <motion.div variants={fade} initial="hidden" whileInView="show" viewport={{ once: true }} className="mb-12 max-w-2xl">
            ...
            <h2 className="font-serif-display mt-4 text-3xl md:text-4xl font-medium tracking-tight">Plans that scale with you</h2>
          ...
          <div className="mt-12 max-w-2xl space-y-4 text-left">
            <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-zinc-500">Common questions</p>
      ...
      <section className="relative z-10 px-6 py-28 border-t border-white/[0.05]">
        <motion.div variants={fade} initial="hidden" whileInView="show" viewport={{ once: true }}
          className="relative mx-auto max-w-3xl text-center border border-white/[0.08] bg-[#121214] border-t-4 border-t-gold p-12 md:p-16">
          <h2 className="font-serif-display relative text-3xl md:text-5xl font-medium tracking-tight leading-tight">{TAGLINE}</h2>
          ...
          <div className="relative z-10 mt-9">
            <button data-testid="footer-cta-btn" onClick={enter} type="button"
              className="group inline-flex items-center gap-2 rounded-lg bg-gold text-black font-medium px-7 py-3 transition-colors hover:bg-gold-hover">
              {authed ? "Open your cockpit" : "Get started"}
              <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </button>
          </div>
          <div className="relative mt-6 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs text-zinc-500">
