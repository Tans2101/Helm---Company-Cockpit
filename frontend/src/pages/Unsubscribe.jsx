...
export default function Unsubscribe() {
  ...
  const body =
    state === "ok"
      ? "You will no longer receive Helm weekly pack or catch-up reminder emails at this address. Transactional messages (invites, task assignments, and high-severity alerts) are unchanged."
      : state === "working"
        ? "One moment — we're updating your preferences."
        : state === "invalid"
          ? "Request a fresh link from a recent Helm email, or contact us and we'll remove you manually."
          : state === "missing"
            ? "Open the Unsubscribe link from a Helm email to opt out of product emails."
